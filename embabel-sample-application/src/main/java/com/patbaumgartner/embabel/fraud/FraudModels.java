 ...
 *
 * Flow: TransactionRequest → [enrichContext] → TransactionContext
 * → [screenForPatterns] → PatternScreening
 * → [decideFraud] → FraudDecision
 *
 * Demo pattern: pure linear pipeline — no conditions, no branching,
 * readOnly enrichment step, single @AchievesGoal at the end.
 */
public class FraudModels {
    ...
    /**
     * Enriched view of the transaction: derived risk signals computed from the
     * raw input without calling an LLM. Produced by a readOnly action.
     */
    public record TransactionContext(String transactionId, String accountId, BigDecimal amount,
            String merchantName, String merchantCategory, String location, String timestamp,
            boolean highValueTransaction, boolean crossBorderTransaction, boolean unusualHour) {
    }

    /**
     * Result of the LLM-based fraud pattern analysis.
     * riskScore: 0.0 (clean) to 1.0 (definite fraud).
     */
