 ...
 *
 * Demo pattern: branching + static cost= on @Action - Two @Condition annotations split
 * the flow at the credit analysis step - Each @Action declares a static cost= so the
 * planner can weigh alternatives - Two @AchievesGoal actions — one per branch - Shows how
 * cost hints influence planner preferences without dynamic methods
 *
 * Plan: LoanRequest → [analyzeCreditProfile] (cost=2.0) → CreditAnalysis →
 * [CAN_AUTO_DECIDE] → [makeAutoDecision] (cost=1.0) → LoanDecision →
 * [REQUIRES_UNDERWRITING] → [conductUnderwriting] (cost=5.0) → UnderwritingAssessment →
 * [makeUnderwrittenDecision] (cost=2.0) → LoanDecision
 */
@Agent(name = "LoanApplicationAgent",
		description = "Assesses the credit risk of a loan application and produces an approve, reject or refer decision.",
		version = "1.0.0")
public class LoanApplicationAgent {
	...
	/**
	 * Step 1: Analyse the applicant's credit profile — DTI ratio, affordability, risk
	 * factors.
	 */
	@Action(description = "Analyse the applicant's credit profile, compute a debt-to-income ratio and affordability score.",
			post = { CAN_AUTO_DECIDE, REQUIRES_UNDERWRITING }, cost = 2.0)
	...
	public boolean canAutoDecide(CreditAnalysis analysis) {
		return "EXCELLENT".equals(analysis.creditRiskCategory()) || "GOOD".equals(analysis.creditRiskCategory())
				|| "VERY_POOR".equals(analysis.creditRiskCategory())
				|| ("POOR".equals(analysis.creditRiskCategory()) && analysis.affordabilityScore() < 30);
	}

	/**
	 * FAIR credit or POOR credit with borderline affordability needs manual underwriting.
	 */
	@Condition(name = REQUIRES_UNDERWRITING)
	public boolean requiresUnderwriting(CreditAnalysis analysis) {
		return "FAIR".equals(analysis.creditRiskCategory())
				|| ("POOR".equals(analysis.creditRiskCategory()) && analysis.affordabilityScore() >= 30);
	}

	/**
	 * Step 2 (underwriting branch): Conduct a deeper underwriting review for borderline
	 * cases.
	 */
	@Action(description = "Conduct a manual underwriting assessment for borderline loan applications.",
			pre = { REQUIRES_UNDERWRITING }, cost = 5.0)
	public UnderwritingAssessment conductUnderwriting(CreditAnalysis analysis, OperationContext context) {
		...
		var prompt = """
				...
				analysis.creditRiskCategory(), String.join(", ", analysis.riskFactors()), analysis.analysisSummary());
	...
	@Action(description = "Make an automatic lending decision for clearly creditworthy or clearly unqualified applicants.",
			pre = { CAN_AUTO_DECIDE }, cost = 1.0)
	public LoanDecision makeAutoDecision(CreditAnalysis analysis, LoanRequest request, OperationContext context) {
		...
		var prompt = """
				...
				analysis.affordabilityScore(), analysis.creditRiskCategory(), String.join(", ", analysis.riskFactors()),
	...
	/**
	 * Step 3 (underwriting branch): Issue a nuanced decision following the underwriting
	 * review.
	 */
	@AchievesGoal(description = "Produce a lending decision for this loan application.")
	@Action(description = "Make a lending decision for a borderline application after a full underwriting assessment.",
			pre = { REQUIRES_UNDERWRITING }, cost = 2.0)
	public LoanDecision makeUnderwrittenDecision(CreditAnalysis analysis, UnderwritingAssessment underwriting,
		...
		log.info("Making underwritten decision for: {} approvableWithConditions={}", analysis.applicantId(),
