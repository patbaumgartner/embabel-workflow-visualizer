 ...
 *
 * Flow: FeedbackRequest → [quickClassify] → SentimentClassification → [deepAnalyze] →
 * SentimentInsight → [respondToCustomer] (@AchievesGoal) → FeedbackResponse
 *
 * Demo pattern: @Cost method — the deepAnalyze action declares a costMethod so the
 * planner can weigh the expense of the LLM call against its value. The cost varies
 * dynamically based on the sentiment score.
 */
public class SentimentModels {
	...
	/**
	 * Result of the fast, cheap first-pass classification. sentiment: POSITIVE, NEGATIVE,
	 * NEUTRAL, MIXED. score: -1.0 (very negative) to +1.0 (very positive).
	...
	/**
	 * Result of the deeper, more expensive analysis step. Includes root-cause topics,
	 * emotional tone, and actionable insights.
	 */
