...
public final class WorkflowModels {
	 ...
	 *                    {@code SUPERVISOR}, or {@code COMPONENT} for
	 *                    {@code @EmbabelComponent} beans
	 * @param opaque      {@code true} if the agent is opaque (internal steps not
	 *                    exposed to
	...
	/**
	 * A step within an agent: an action, condition, goal, cost function, or LLM
	 * tool.
	 *
	 * @param name               display name (annotation {@code name} or method
	 *                           name)
	 ...
	 * @param inputs             input parameter type names (excluding framework
	 *                           types like
	 ...
	 *                           {@code @Action(costMethod = ...)}; {@code null} if
	 *                           not set
	 * @param valueMethod        name of the {@code @Cost} method referenced via
	 *                           {@code @Action(valueMethod = ...)}; {@code null} if
	 *                           not set
	 * @param cost               static cost declared via
	 *                           {@code @Action(cost = ...)}; {@code null}
	 *                           when left at the default ({@code 0.0})
	 * @param value              static value declared via
	 *                           {@code @Action(value = ...)}; {@code null}
	 *                           when left at the default ({@code 0.0})
	 * @param goalValue          goal value declared via
	 *                           {@code @AchievesGoal(value = ...)};
	 *                           {@code null} when left at the default ({@code 0.0})
	 * @param possibleOutputs    when a method's declared return type is
	 *                           {@code Object}, this
	 *                           list holds the concrete types it may actually
	 *                           return (inferred from {@code @State}
	 *                           inner-record component types on the same agent
	 *                           class); {@code null} otherwise
	 * @param canRerun           {@code true} when {@code @Action(canRerun = true)}
	 *                           — the planner
	 *                           may schedule this step more than once
	 * @param readOnly           {@code true} when {@code @Action(readOnly = true)}
	 *                           — step does not
	 *                           mutate the blackboard
	 * @param outputBinding      explicit blackboard binding name set via
	 *                           {@code @Action(outputBinding = "...")};
	 *                           {@code null} if not set
	 * @param clearBlackboard    {@code true} when
	 *                           {@code @Action(clearBlackboard = true)}
	 * @param tags               tags from {@code @AchievesGoal(tags = {...})}
	 * @param examples           example prompts from
	 *                           {@code @AchievesGoal(examples = {...})}
	 * @param llmTool            {@code true} if the method carries
	 *                           {@code @LlmTool}, making it
	 *                           directly callable by the LLM
	 * @param llmToolDescription description from
	 *                           {@code @LlmTool(description = "...")}
	 * @param exportedRemote     {@code true} when the goal is published as a remote
	 *                           MCP tool
	 *                           via
	 *                           {@code @AchievesGoal(export = @Export(remote = true, ...))}
	 * @param exportName         explicit export name from
	 *                           {@code @Export(name = "...")};
	 *                           {@code null} if not set
	 */
	public record WorkflowStep(String name, String type, String description, String method, List<String> pre,
			...
			boolean readOnly,
			String outputBinding, boolean clearBlackboard, List<String> tags, List<String> examples, boolean llmTool,
			String llmToolDescription, boolean exportedRemote, String exportName) {
	}
